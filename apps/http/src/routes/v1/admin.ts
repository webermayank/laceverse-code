import { Avatar } from "./../../../../../node_modules/.prisma/client/index.d";
import { Router } from "express";
import { adminMiddleware } from "../../middleware/user";
import {
  AddElementSchema,
  CreateAvatarSchema,
  CreateElementSchema,
...
adminRouter.post("/map", adminMiddleware, async (req, res) => {
  ...
  const map = await client.map.create({
    data: {
      ...
      height: parseInt(parseData.data.dimensions.split("x")[0]),
      thumbnail: parseData.data.thumbnail,
      mapElements: {
        create: parseData.data.defaultElement.map((e) => ({
          elementId: e.elemntId,
          x: e.x,
          y: e.y,
        })),
      },
